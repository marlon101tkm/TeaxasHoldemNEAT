...
public class PlayerTP : Player
{
    public PlayerTP(string playerName, int startingChips, PlayerType playerType, int index) : base(playerName, startingChips, playerType, index)
    {
    }

    public override void TomaDecisao()
    {
        //_currentGame.GameController.printLog("Player TP");
        double hs = forcaMao(_holeCards, _currentGame.Board.FullBoard);
        _currentGame.GameController.printLog("Player TP força da mão: " + hs);
