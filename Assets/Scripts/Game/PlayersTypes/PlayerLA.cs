...
public class PlayerLA : Player
{
    public PlayerLA(string playerName, int startingChips, PlayerType playerType, int index) : base(playerName, startingChips, playerType, index)
    {
    }

    public override void TomaDecisao()
    {
        //_currentGame.GameController.printLog("Player LA");
        double hs = forcaMao(_holeCards, _currentGame.Board.FullBoard);
        _currentGame.GameController.printLog("Player LA força da mão: "+ hs );
