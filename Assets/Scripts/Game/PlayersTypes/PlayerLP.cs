...
public class PlayerLP : Player
{
    public PlayerLP(string playerName, int startingChips, PlayerType playerType, int index) : base(playerName, startingChips, playerType, index)
    {
    }

    public override void TomaDecisao()
    {
        //_currentGame.GameController.printLog("Player LP");
        double hs = forcaMao(_holeCards, _currentGame.Board.FullBoard);
        _currentGame.GameController.printLog("Player LP força da mão: " + hs);
