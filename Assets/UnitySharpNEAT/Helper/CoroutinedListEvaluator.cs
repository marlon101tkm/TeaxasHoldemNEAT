        ...
        where TPhenome : class
    {
       ...
       TableFillerController tableFillerController = GameObject.Find("TableFiller").GetComponent<TableFillerController>();
        ...
        private IEnumerator EvaluateList(IList<TGenome> genomeList)
        {
            ...
            Reset();
            _neatSupervisor.GenerationEnded = true;
            //yield return new WaitUntil(() => GameMenager.Instance.getIfTrialEnded());
            ...
            for ( indice = 0; indice < _neatSupervisor.Trials; indice++)
            //while (indice < _neatSupervisor.Trials)
            {
                
                //GameMenager.Instance.CurrentTrial = indice;
                //_neatSupervisor.CurrentTrial = indice;
                //Utility.Log("Antes do trial " + _neatSupervisor.CanStartNewHand);
                Utility.Log("UnityParallelListEvaluator.EvaluateList -- Begin Trial " + (indice + 1));
               // GameMenager.Instance.SetAllTrialEnded();
                //GameMenager.Instance.CanPlayNextHand = true;

                //GameMenager.Instance.atualizaNeatTrialInprogress();
                //_neatSupervisor.SetTempoTrial(true);
                ...
                if (indice == 0)
                {
                   ...
                   // GameMenager.Instance.AllUnitsAreActtive = true;
                }
                ...
                //Utility.Log("Estado do tempo do Trial: " + GameMenager.Instance.getIfTrialEnded() + " Can play new hand " + GameMenager.Instance.getIfCanPlayHand() );

              
                Utility.Log("Antes to yield " + _neatSupervisor.CanStartNewHand);
                yield return new WaitWhile(() => _neatSupervisor.CanStartNewHand);
                ...
                foreach (TGenome genome in dict.Keys)
                {
                    ...
                    if (phenome != null)
                    {
                        ...
                        fitnessDict[genome][indice] = fitnessInfo;
            ...
            //_neatSupervisor.CanStartNewHand = true;
            _neatSupervisor.GenerationEnded = false;
            //GameMenager.Instance.SetCanPlayHand();
            ...
            //Utility.Log("numero de populacao"+ dict.Count );
           // GameMenager.Instance.CreateText(_neatSupervisor.TxtFileName, GameMenager.Instance.CurrentGeneration + ";" + (maiorFitness/dict.Count) + "\n", "Geracao;Media Fitness");
            //GameMenager.Instance.AllUnitsAreActtive = false;
            yield return 0;
        }
