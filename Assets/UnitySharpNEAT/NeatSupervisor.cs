...
namespace UnitySharpNEAT
{
    ...
    public class NeatSupervisor : MonoBehaviour
    {
        ...
        //private bool _tempoTrial = true;
        private bool _canStartNewHand = true;
        public bool CanStartNewHand
        {
            get { return _canStartNewHand; }
            set { _canStartNewHand = value; }
        }

        private bool _generationEnded = true;
        public bool GenerationEnded
        {
            get { return _generationEnded; }
            set { _generationEnded = value; }
        }


        private bool _allAiFinished = true;
        public bool AllAiFinished
        {
            get { return _allAiFinished; }
            set { _allAiFinished = value; }
        }
        //public void SetTempoTrial (bool flag )
        //{
        //    _tempoTrial = flag;
        //}
        //public bool GetTempoTrial()
        //{
        //    return _tempoTrial;
        //}
        ...
        //public bool TempoTrial {
        //    get { return _tempoTrial; }
        //    set { _tempoTrial = value; }
        //}
        //public bool FimTrial
        //{
        //    get { return _fimTrial;  }
        //    set { _fimTrial = value ; }
        //}
        ...
        public void StopEvolution()
        {
            ...
            //GameMenager.Instance.SetAllTrialEndedFalse();
            if (EvolutionAlgorithm != null && EvolutionAlgorithm.RunState == SharpNeat.Core.RunState.Running)
            {
               //TableFillerController tabfi = GameObject.Find("TableFiller").GetComponent<TableFillerController>();
                //foreach (var tab in tabfi.getTables())
                //{
                //    Destroy(tab);
                //}
        ...
        /*
        public UnitController RegisterInstance()
        {
            
            UnitController controller = Instantiate(_unitControllerPrefab);

            int numPlayers =  gameController.CurrentGame.NumPlayers;

            

            for (int i = 0 ; i < numPlayers ; i++)
            {
               controller = gameController.transform.Find("Players").transform.Find("Player" + i).gameObject.GetComponent<Player>();
                if (controller != null)
                {
                    Debug.Log("Jogador " + i + "Instanciado");

                }
                    if (!_unusedUnitsPool.Contains(controller))
                    {
                        return controller;
                    }
        ...
        */
        ...
        private UnitController InstantiateUnit(IBlackBox box)
        {
            ...
            // UnitController controller = RegisterInstance();

            //Debug.Log("Instanciou unidades NEAT");
            if (_spawnParent != null)
            {
                //controller.transform.parent = _spawnParent;
                controller.transform.SetParent(_spawnParent);//, false);
            }
            else
            {
                //controller.transform.parent = this.transform;
                controller.transform.SetParent(this.transform);//, false);
            }
